...
{
    bool anyDiagonallyAdjacent = false;
    for (int k = 0; k < m_nFlatulans; k++)
    {
        Flatulan* fp = m_flatulans[k];
        fp->move();
        int rowdiff = fp->row() - m_player->row();
        int coldiff = fp->col() - m_player->col();
        bool oneRowAway = (rowdiff == 1  ||  rowdiff == -1);
        bool oneColAway = (coldiff == 1  ||  coldiff == -1);
          // if orthogonally adjacent
        if  ((rowdiff == 0  &&  oneColAway) || (coldiff == 0  &&  oneRowAway))
            m_player->getGassed();
        else if (oneRowAway  &&  oneColAway)  // diagonally adjacent
            anyDiagonallyAdjacent = true;
    }
    if (anyDiagonallyAdjacent)
